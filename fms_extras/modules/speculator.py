...
class Speculator(nn.Module):
    ...
    def generate_suffixes(self, state, ind, topk=[5, 4, 3], n=5):
        ...
        assert (
            len(topk) == self.n_heads
        ), f"You must provide a topk number for each head ({self.n_heads} heads, {len(topk)} provided)"
        ...
        return out.gather(
            1, best_guesses.unsqueeze(2).expand(-1, -1, self.n_heads)
        )  # b n h
    ...
    def forward(self, state, inds):
        ...
        Produces self.n_heads predicted tokens for each token embedding in state.
        Inds requires self.n_heads extra tokens on the right to "simulate" recursive behavior for end positions.
        """
        # state: b n d
